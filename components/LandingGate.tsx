...
export const LandingGate: React.FC<LandingGateProps> = ({ onLogin }) => {
  ...
  const guestLogin = () => onLogin({
    ...
    email: isWissums ? 'guest@wissums.com' : 'guest@storyscribe.app', org_id: DEMO_ORG_ID, is_admin: false,
  });
  ...
  const handleAdminBypass = async () => {
    ...
    try {
      ...
      const admin = { id: 'admin-001', email: isWissums ? 'admin@wissums.com' : 'admin@storyscribe.app', name: 'Archive Curator', org_id: DEMO_ORG_ID, is_admin: true };
      localStorage.setItem(isWissums ? 'wissums_customer' : 'storyscribe_customer', JSON.stringify(admin));
      onLogin(admin);
